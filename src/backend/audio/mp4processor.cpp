...
void mp4Processor::addtoFrame(uint8_t *V) {
  ...
  for (i = 0; i < nbits / 8; i++) {  // in bytes
    temp = 0;
    for (j = 0; j < 8; j++) temp = (temp << 1) | (V[i * 8 + j] & 01);
  ...
  if (blocksInBuffer >= 5) {
    ...
    if (fc.check(&frameBytes[blockFillIndex * nbits / 8]) &&
        (processSuperframe(frameBytes.data(), blockFillIndex * nbits / 8))) {
      ...
      blocksInBuffer = 4;
      frameErrors++;
      if (errorReportHandler) errorReportHandler(1, 1, totalFrameCount, ctx);
    }
  }
}
//
bool mp4Processor::processSuperframe(uint8_t frameBytes[], int16_t base) {
  ...
  for (j = 0; j < RSDims; j++) {
    ...
    if (ler < 0) {
      ++rsErrors;
      if (errorReportHandler) errorReportHandler(2, 1, 0, ctx);
      return false;
    }
    for (k = 0; k < 110; k++) outVector[j + k * RSDims] = rsOut[k];
  }
  //
  ...
  for (i = 0; i < num_aus; i++) {
    ...
    if (check_crc_bytes(&(outVector.data())[au_start[i]], aac_frame_length)) {
      bool err;
      //
      ...
      memcpy(&fileBuffer[7], &outVector[au_start[i]], aac_frame_length);
      if (soundOut != nullptr)
        (soundOut)((int16_t *)(&fileBuffer[0]), aac_frame_length + 7, 0, false,
                   nullptr);
#else
      ...
      if (err) {
        ++aacErrors;
        if (aacErrors && errorReportHandler) errorReportHandler(3, 1, 0, ctx);
      }
      if (++aacFrames > 25) {
      ...
      if (errorReportHandler) errorReportHandler(6, 1, 0, ctx);
    }
  }
  return true;
}
...
void mp4Processor::show_frameErrors(int s) { (void)s; }

void mp4Processor::show_rsErrors(int s) { (void)s; }

void mp4Processor::show_aacErrors(int s) { (void)s; }

void mp4Processor::isStereo(bool b) { (void)b; }

void mp4Processor::buildHeader(int16_t framelen, stream_parms *sp,
                               uint8_t *header) {
