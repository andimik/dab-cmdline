...
mp2Processor::~mp2Processor() { delete[] MP2frame; }
//
...
void mp2Processor::read_samples(struct quantizer_spec *q, int scalefactor,
  ...
  if (q->grouping) {  // decode grouped samples
    ...
    for (idx = 0; idx < 3; ++idx) sample[idx] = get_bits(q->cw_bits);
  }
...
int32_t mp2Processor::mp2decodeFrame(uint8_t *frame, int16_t *pcm,
  ...
  if (numberofFrames >= 50) {
    if (mscQuality != nullptr) mscQuality(2 * (50 - errorFrames), 0, 0, ctx);
    numberofFrames = 0;
    errorFrames = 0;
  }
  ...
  if (sampling_frequency == 3) return 0;
  ...
  *stereo = ((mode == JOINT_STEREO) || (mode == STEREO));
  ...
  if (!pcm) return frame_size;  // no decoding
  ...
  for (sb = 0; sb < sblimit; ++sb) {
    for (ch = 0; ch < nch; ++ch)
      if (allocation[ch][sb]) scfsi[ch][sb] = get_bits(2);

    if (mode == MONO) scfsi[1][sb] = scfsi[0][sb];
  }
    ...
    for (gr = 0; gr < 4; ++gr) {
      ...
      for (sb = bound; sb < sblimit; ++sb) {
        read_samples(allocation[0][sb], scalefactor[0][sb][part],
                     &sample[0][sb][0]);
        for (idx = 0; idx < 3; ++idx) sample[1][sb][idx] = sample[0][sb][idx];
      }

      for (ch = 0; ch < 2; ++ch)
        for (sb = sblimit; sb < 32; ++sb)
          for (idx = 0; idx < 3; ++idx) sample[ch][sb][idx] = 0;
      ...
      for (idx = 0; idx < 3; ++idx) {
        ...
        for (ch = 0; ch < 2; ++ch) {
          ...
          // apply window
          for (i = 0; i < 512; ++i) U[i] = (U[i] * D[i] + 32) >> 6;

          // output samples
          for (j = 0; j < 32; ++j) {
            sum = 0;
            for (i = 0; i < 16; ++i) sum -= U[(i << 5) + j];
            sum = (sum + 8) >> 4;
    ...
    if (MP2Header_OK == 2) {
...
#ifdef AAC_OUT
        soundOut((int16_t *)(&MP2frame[0]), MP2bitCount, 0, false, nullptr);
#else
        if (mp2decodeFrame(MP2frame, sample_buf, &stereo)) {
          output(sample_buf, 2 * (int32_t)KJMP2_SAMPLES_PER_FRAME, baudRate,
                 stereo);
        } else {
          ++errorFrames;
      ...
      if (v[i] == 01) {
        if (++MP2headerCount == 12) {
          MP2bitCount = 0;
          for (j = 0; j < 12; j++) addbittoMP2(MP2frame, 1, MP2bitCount++);
          MP2Header_OK = 1;
        }
      } else
        MP2headerCount = 0;
...
void mp2Processor::output(int16_t *buffer, int size, int rate, bool stereo) {
  if (soundOut != nullptr) soundOut(buffer, size, rate, stereo, ctx);
}
