...
void printOptions(const unsigned default_nFIBsToFlush) {
  ...
  fprintf(stderr,
          "\t-b <nFIBsToFlush>   number for FIBs to flush decoded output\n");
  fprintf(stderr, "\t                    default: %u for 19.2 sec\n",
          default_nFIBsToFlush);
  fprintf(stderr,
          "\t-f <fic-file>       filename of raw/binary FIC input file\n");
  fprintf(stderr, "\n");
...
void flush_fig_processings() { flush_fig0_ext6(); }
...
static void fib_dataHandler(const uint8_t *fib) {
  ++fibCallbackNo;
  const uint8_t *d = fib;
  int processed = 0;
  while (processed < 30) {
...
int main (int argc, char **argv) {
  ...
  int opt, verbosity = 0;
  const unsigned default_nFIBsToFlush =
      200U * 36U;  // mode I has 36 FIBs per DAB/OFDM frame of size 96 ms
  unsigned nFIBsToFlush =
      default_nFIBsToFlush;  // == 200 frames == 19.2 sec with 7200 FIBs of 32
                             // Bytes each
    ...
    switch (opt) {
      case 'b':
        ...
        if (nFIBsToFlush <= 0) {
          ...
          fprintf(
              stderr,
              "invalid argument nFIBsToFlush = '%s'. setting to default %u\n",
  ...
  if (verbosity)
    fprintf(stderr,
            "decodefic - Copyright 2020 Hayati Ayguen <h_ayguen@web.de>\n");
