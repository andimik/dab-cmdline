...
tii_element::tii_element(int16_t subId, int16_t TD, float latitudeOffset,
...
void tii_table::add_element(tii_element *t) {
  ...
  for (i = 0; i < offsets.size(); i++)
    if (offsets[i].subId == t->subId) return;

  offsets.push_back(*t);
}

std::complex<float> tii_table::get_coordinates(int16_t mainId, int16_t subId,
                                               bool *success) {
  uint16_t i;
  float x, y;

  *success = false;
  if (this->mainId != mainId) return std::complex<float>(0, 0);

  for (i = 0; i < offsets.size(); i++) {
    if (offsets[i].subId != subId) continue;
...
std::complex<float> tii_table::get_coordinates(int16_t mainId, int16_t subId,
  ...
  *success = false;
  ...
  for (i = 0; i < offsets.size(); i++) {
    // subId == -1 => deliver first available offset
    if (offsets[i].subId != subId && subId != -1) continue;
