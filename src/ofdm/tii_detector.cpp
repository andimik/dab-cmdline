...
#define NUM_MINS_FOR_NOISE \
  4 /* use lowest 4 carrier pairs to determine noise level */
...
TII_Detector::TII_Detector(uint8_t dabMode)
    : phaseTable(dabMode), params(dabMode), my_fftHandler(dabMode) {
  int16_t i;
  float Phi_k;
  double winSum = 0.0;
  double winElem;
  ...
  for (i = 0; i < T_u; i++) {
    winElem = (0.42 - 0.5 * cos(2 * M_PI * (float)i / T_u) +
...
//	   Zm (0, k) = A (c, p) (k) e^jPhi (k) + A (c, p) (k - 1) e ^jPhi (k -
// 1)
//
...
static uint8_t invTable[256];

static void initInvTable() {
  int i;
  for (i = 0; i < 256; ++i)
    invTable[i] = 100;  // initialize with an invalid mainId!
  for (i = 0; i < 70; ++i) invTable[table[i]] = i;
}

static uint8_t bits[] = {128, 64, 32, 16, 8, 4, 2, 1};
static inline uint8_t getbit(uint8_t value, int16_t bitPos) {
  return value & bits[bitPos] ? 1 : 0;
}

static inline uint8_t delta(int16_t a, int16_t b) { return a == b ? 1 : 0; }
//
...
int16_t TII_Detector::A(uint8_t dabMode, uint8_t c, uint8_t p, int16_t k) {
  if (dabMode == 2) return A_mode_2(c, p, k);
  if (dabMode == 4) return A_mode_4(c, p, k);
...
void TII_Detector::addBuffer(std::vector<std::complex<float>> v, float alfa,
  ...
  for (int j = 1; j < T_u; ++j) {
    ...
    if (n > timeMaxNorm) timeMaxNorm = n;
  }
#endif

  // windowing + FFT
  for (i = 0; i < T_u; i++) fft_buffer[i] = v[i];  // cmul (v [i], window [i]);
  ...
  for (int j = 1; j < T_u; ++j) {
    if (P_tmpNorm[j] > specMaxNorm) specMaxNorm = P_tmpNorm[j];
  }
  ...
  if (alfa < 0.0F || isFirstAdd) {
    for (int j = 0; j < T_u; ++j) P_allAvg[j] += P_tmpNorm[j];
    for (i = 0; i < T_u; i++) theBuffer[i] += fft_buffer[(T_u / 2 + i) % T_u];
    isFirstAdd = false;
    ++numUsedBuffers;
  } else {
    ...
    for (i = 0; i < T_u; i++)
      theBuffer[i] =
          alpha * theBuffer[i] + beta * fft_buffer[(T_u / 2 + i) % T_u];
    ++numUsedBuffers;
  }
}
...
int16_t TII_Detector::find_C(int16_t mainId) {
  ...
  if (mainId < 0) return -1;

  //	fprintf (stderr, "the carrier is %d\n", startCarrier);
  //
  //	The "c" value is in the range 1 .. 23
  for (i = 1; i < 24; i++) {
    corrTable[i] = correlate(theBuffer, startCarrier + 2 * i, pattern);
  }
...
void TII_Detector::processNULL(int16_t *mainId, int16_t *subId) {
  ...
  for (i = -carriers / 2; i < -carriers / 4; i++)
    avg += abs(real(theBuffer[T_u / 2 + i] * conj(theBuffer[T_u / 2 + i + 1])));
  avg /= (carriers / 4);
  ...
  *subId = -1;
  ...
  for (i = 0; i < 70; i++) {
    if ((startCarrier < theTable[i].carrier) ||
      ...
      continue;
    float corr = correlate(theBuffer, startCarrier, theTable[i].pattern);
    if (corr > maxCorr) {
      maxCorr = corr;
      *mainId = i;
...
static inline float powerRatio(float s, float n) {
  if (s > n) {
    if (s > n * 1E20F) return 1E20F;
  } else {
    if (s * 1E20F < n) return 1E-20F;
  }
  return s / n;
}
...
void TII_Detector::processNULL_ex(int *pNumOut, int *outTii, float *outAvgSNR,
  ...
  *pNumOut = 0;
  if (params.get_dabMode() != 1) return;
  ...
  //   don't intermix with exponential averaging ..
  {
    static int modeOneCarrierFFTidx[5] = {
        (-768 + 2048) % 2048, (-384 + 2048) % 2048,
        // center carrier 0 unused!
        (1 + 2048) % 2048, (385 + 2048) % 2048, (769 + 2048) % 2048};
  ...
  for (groupNo = 0; groupNo < NUM_GROUPS; ++groupNo) {
    ...
    P_grpPairNoise[groupNo] =
        std::accumulate(P + (NUM_SUBIDS - NUM_MINS_FOR_NOISE), P + NUM_SUBIDS,
                        0.0F) /
        NUM_MINS_FOR_NOISE;
    ...
    for (j = 0; j < MAX_NUM_TII; ++j) {
      // array size is 20, cause lowest 4 elements are already at the tail (of
      // 24 elements)
      ...
      if ((P[j] < MIN_SNR_POWER_RATIO * P_grpPairNoise[groupNo]) ||
          (P_avg[C[j]] <
           (MIN_SNR_POWER_RATIO / 2.0F) * P_grpPairNoise[groupNo]) ||
          (P_avg[C[j] + 1] <
           (MIN_SNR_POWER_RATIO / 2.0F) * P_grpPairNoise[groupNo]))
        break;
  ...
  for (int subID = 0; subID < NUM_SUBIDS; ++subID) {
    if (num_subIDs[subID] <
        NUM_SUBIDS_IN_PATTERN)  // skip if not found minimum 4 times
      continue;
    ...
    if (numGroupsWithSubID < NUM_SUBIDS_IN_PATTERN) continue;
    ...
    for (j = 0; j <= NUM_SUBIDS_IN_PATTERN; ++j) {
      ...
      if (j < NUM_SUBIDS_IN_PATTERN) {
        ...
        if (grpSNR < minGrpSNR || minGrpSNR < 0.0F) minGrpSNR = grpSNR;
#endif
      }
    }
    ...
    if (minSNR >= MIN_SNR_POWER_RATIO) {
      ...
      float nextMainSNR =
          MAX_SNR;  // max +200 dB as default, if there is no other group
...
float TII_Detector::correlate(std::vector<complex<float>> v,
                              int16_t startCarrier, uint64_t pattern) {
  int16_t realIndex;
  int16_t i;
  int16_t carrier;
  float s1 = 0;
  ...
  s1 = abs(
      real(v[T_u / 2 + startCarrier] * conj(v[T_u / 2 + startCarrier + 1])));
  for (i = 0; i < 15; i++) {
