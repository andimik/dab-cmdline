...
void *dabInit(deviceHandler *theDevice, uint8_t Mode,
              syncsignal_t syncsignal_Handler, systemdata_t systemdata_Handler,
              ensemblename_t ensemblename_Handler,
              programname_t programname_Handler,
              fib_quality_t fib_quality_Handler, audioOut_t audioOut_Handler,
              dataOut_t dataOut_Handler, bytesOut_t bytesOut_Handler,
...
void dabExit(void *Handle) { delete (dabProcessor *)Handle; }

void dabStartProcessing(void *Handle) { ((dabProcessor *)Handle)->start(); }

void dabReset(void *Handle) { ((dabProcessor *)Handle)->reset(); }

void dabStop(void *Handle) { ((dabProcessor *)Handle)->stop(); }
...
void dab_setTII_handler(void *Handle, tii_t tii_Handler, tii_ex_t tii_ExHandler,
  ...
  return ((dabProcessor *)Handle)
      ->setTII_handler(tii_Handler, tii_ExHandler, tii_framedelay, alfa,
                       resetFrameCount);
}
...
void dab_getCoordinates(void *Handle, int16_t mainId, int16_t subId,
                        float *latitude, float *longitude, bool *success,
                        int16_t *pMainId, int16_t *pSubId, int16_t *pTD) {
  std::complex<float> r =
      ((dabProcessor *)Handle)
          ->get_coordinates(mainId, subId, success, pMainId, pSubId, pTD);
  *latitude = r.real();
  *longitude = r.imag();
}

uint8_t dab_getExtendedCountryCode(void *Handle, bool *success) {
  return ((dabProcessor *)Handle)->getECC(success);
}

uint8_t dab_getInternationalTabId(void *Handle, bool *success) {
  return ((dabProcessor *)Handle)->getInterTabId(success);
}

void dataforAudioService(void *Handle, const char *name, audiodata *d, int o) {
